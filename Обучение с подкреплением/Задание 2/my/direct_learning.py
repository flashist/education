...
Параметры: α=0.1, γ=1.0, ε=0.2, эпизодов={episodes_count}.
""")
    ...
    if q_01[RIGHT] > q_01[DOWN]:
        ...
        print("  Вывод: агент предпочитает обходить телепорт — телепорт не сокращает путь,")
        print("         но добавляет неопределённость: из (2,0) можно уйти дальше от цели.")
    print()
    ...
    if q_01[RIGHT] > q_01[DOWN]:
        ...
        print("  Вывод: агент предпочитает обходить телепорт — его исход непредсказуем:")
        print("         иногда он переносит ближе к цели (напр. (2,1), (1,2)),")
        print("         но в среднем матожидание хуже, чем у прямого маршрута.")
    print()
    ...
    # ── Вывод ─────────────────────────────────────────────────────────────
    fixed_uses_teleport  = fixed_agent.Q[(0, 1)][RIGHT]  > fixed_agent.Q[(0, 1)][DOWN]
    random_uses_teleport = random_agent.Q[(0, 1)][RIGHT] > random_agent.Q[(0, 1)][DOWN]

    def teleport_verdict(uses):
        return "использует телепорт как ярлык" if uses else "обходит телепорт, предпочитая прямой путь"
...
Фиксированный телепорт: агент {teleport_verdict(fixed_uses_teleport)}.
Случайный телепорт    : агент {teleport_verdict(random_uses_teleport)}.
